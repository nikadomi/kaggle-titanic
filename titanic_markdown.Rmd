...
The highest survival rate is observed in 1st class (almost 63%) and the lowest in 3rd: 24%. Let's see who these classes consist of:
...
## Name Variable
From other titanic tutorials that I reviewed, it was clear that getting the title out of the *Name* variable helped getting a better prediction a lot. So I'm going to try that now.
```{r, message=FALSE, warning=FALSE}
...
## Age Variable
There are 263 missing values in *Age* variable that we need to take care of. 
```{r, message=FALSE, warning=FALSE}
sum(is.na(all$Age))
```
